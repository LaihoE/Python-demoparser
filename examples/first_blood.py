...
def first_bloods(file):
    ...
    for event in game_events:
        ...
        if event["event_name"] == "round_start":
            round_starts.append(event["tick"])

    deaths.sort(key=lambda x: x["tick"])
    round_starts.sort()

    first_bloods = []
    for start_tick in round_starts:
        for death in deaths:
            if death["tick"] > start_tick:
                first_bloods.append(death)
                break

    victims = [x["player_id"] for x in first_bloods]
    attackers = [x["attacker_id"] for x in first_bloods]

    total_attackers.extend(attackers)
    total_victims.extend(victims)
    return total_attackers, total_victims
...
if __name__ == "__main__":
    ...
    #print(attackers)
    attackers = Counter(attackers)
    victims = Counter(victims)

    print("BENU FIRST BLOODS KILLS:", attackers["76561198134270402"])
    print("BENU FIRST BLOODS DEATHS:", victims["76561198134270402"])

    print("****")

    print("EMIL FIRST BLOODS KILLS:", attackers["76561198194694750"])
    print("EMIL FIRST BLOODS DEATHS:", victims["76561198194694750"])